...
Available on Teams apps on Windows, Mac, Linux, Web, iOS, and Android.

## Loop components and .fluid files

Loop components created in Teams are backed by a .loop file stored in the creator's OneDrive. Being a file in OneDrive means that users can create, discover, and manage Loop components (.fluid files) as easily as any Office document. Note: .fluid files do not currently support eDiscovery.
